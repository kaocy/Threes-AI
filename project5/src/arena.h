...
public:
    ...
    public:
        match(const std::string& id, std::shared_ptr<agent> play, std::shared_ptr<agent> evil) : 
            id(id), play(play), evil(evil), prev_action(action::place(0, 0)) {}
        std::string name() const { return id; }
        void set_prev_action(action move) {
            prev_action = move;
        }

        action take_action() {
            agent& who = take_turns(*play, *evil);
            action move = who.take_action(state(), prev_action);
            return move;
        }
